...
class BreastMNISTDataset:
    ...
    def _preprocess_images(self):
        ...
        if self.normalize:
            if self.normalize_mode == "minmax":
                ...
                self.train_images = (
                    self.train_images - self.train_mean
                ) / self.train_std
    ...
    def get_split(
        self, split: str = "train", fraction: float = 0.75, return_labels: bool = True
    ) -> Tuple[np.ndarray, ...]:
        """
    ...
    def get_all_splits(
        self, train_fraction: float = 0.75, val_fraction: float = 0.25
        ...
        X_test, y_test = self.get_split(
            "test", fraction=1.0
        )  # Always use full test set
