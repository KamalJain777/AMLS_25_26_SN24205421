...
def plot_sample_images(
    ...
    for label in unique_labels:
        ...
        indices.extend(np.random.choice(label_indices, n_samples_label, replace=False).tolist())
...
def plot_class_distribution(
    ...
    for idx, (split_name, labels) in enumerate(splits):
        ...
        bars = axes[idx].bar([class_names[u] for u in unique], counts, color=[colors[u] for u in unique])
...
def plot_confusion_matrices(
    ...
    for idx, (mode, results) in enumerate(results_dict.items()):
        if 'test' in results and 'confusion_matrix' in results['test']:
            ...
            axes[idx].set_title(f'{mode.upper()} Features\nTest Set', fontsize=12, fontweight='bold')
...
def plot_metrics_comparison(
    ...
    for metric_idx, metric in enumerate(metrics):
        ...
        if values:
            ...
            axes[metric_idx].set_title(metric.replace('_', ' ').title(), fontsize=11, fontweight='bold')
...
def plot_feature_comparison_table(
    ...
    plt.title('Feature Modes Performance Comparison (Test Set)', 
...
def plot_augmentation_comparison(
    ...
    if 'test' not in results_without_aug or 'metrics' not in results_without_aug['test']:
    ...
    bars1 = ax.bar(x - width/2, without_values, width, label='Without Augmentation', 
                   color='#FF6B6B', alpha=0.8)
    bars2 = ax.bar(x + width/2, with_values, width, label='With Augmentation', 
    ...
    ax.set_title('Performance Comparison: Without vs With Data Augmentation (Test Set)', 
                 fontsize=14, fontweight='bold', pad=20)
...
def plot_augmentation_confusion_matrices(
    ...
    for idx, (results, title_suffix) in enumerate([(results_without_aug, 'Without Augmentation'),
                                                    (results_with_aug, 'With Augmentation')]):
        if 'test' not in results or 'confusion_matrix' not in results['test']:
            axes[idx].text(0.5, 0.5, 'Confusion matrix not available', 
        ...
        axes[idx].set_title(f'{title_suffix}\n(Test Set)', fontsize=12, fontweight='bold')
        axes[idx].set_xticklabels(['Benign', 'Malignant'])
        axes[idx].set_yticklabels(['Benign', 'Malignant'])
    
    plt.suptitle('Confusion Matrices: Without vs With Augmentation', 
...
def plot_augmentation_roc_curves(
    ...
    # Plot ROC curves
    ax.plot(fpr_without, tpr_without, label=f'Without Augmentation (AUC = {auc_without:.3f})',
           linewidth=2, color='#FF6B6B')
    ax.plot(fpr_with, tpr_with, label=f'With Augmentation (AUC = {auc_with:.3f})',
           linewidth=2, color='#4ECDC4')
    ax.plot([0, 1], [0, 1], 'k--', label='Random Classifier', linewidth=1, alpha=0.5)
    
    ax.set_xlabel('False Positive Rate', fontsize=12)
    ax.set_ylabel('True Positive Rate', fontsize=12)
    ax.set_title('ROC Curves: Without vs With Augmentation', fontsize=14, fontweight='bold', pad=20)
